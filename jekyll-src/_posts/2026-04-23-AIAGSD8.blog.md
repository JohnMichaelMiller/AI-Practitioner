...
task_durations:
  ...
  - task: "draft AIAGSD8 post and CODE copy"
    duration: "00:07:00"
  - task: "provenance logging and repo updates"
    duration: "00:02:00"
...
<figure>
  <img src="https://epsenterprise.blob.core.windows.net/permanent-files/FileAttachments/c8bb814e_635f_4b3b_a5c7_691b480818f1/AIAGSD8_Header_Large.png" alt="Foundational vertical slices establishing shared kernel and reference data before broader implementation" />
...
This is the eighth post in the series on AI-assisted greenfield software development. It builds on <a href="https://blog.pdata.com/AIAGSD1" target="_blank">Part 1: Business Requirements</a> and the subsequent posts in the series. If you have not read the earlier posts, start there first, because Part 8 assumes the repository already has prompts, instruction files, agent roles, provenance rules, and a dependency-aware implementation plan.
...
<figure>
  <img src="https://epsenterprise.blob.core.windows.net/permanent-files/FileAttachments/261b2d0c_2566_4ced_b505_5072082778f5/AIAGSD8_Slice_Sequence.png" alt="AIAGSD8_Slice_Sequence.png" />
  <figcaption>The detailed slice implementation sequence, showing the interactions between the slice coordinator, backend domain, data persistence, and testing verification components.</figcaption>
</figure>
...
That change now shows up directly in the agent guidance for [`data-persistence`](https://github.com/j0hnnymiller/zeus.academia.3b/blob/main/.github/agents/data-persistence.agent.md), [`testing-verification`](https://github.com/j0hnnymiller/zeus.academia.3b/blob/main/.github/agents/testing-verification.agent.md), and [`slice-verifier`](https://github.com/j0hnnymiller/zeus.academia.3b/blob/main/.github/agents/slice-verifier.agent.md). These agents are no longer allowed to treat schema-changing work as complete based solely on code-level checks.
...
This matrix sharpens agent responsibilities. The [`backend-domain`](https://github.com/j0hnnymiller/zeus.academia.3b/blob/main/.github/agents/backend-domain.agent.md) agent owns the aggregate and handler behavior. The [`data-persistence`](https://github.com/j0hnnymiller/zeus.academia.3b/blob/main/.github/agents/data-persistence.agent.md) agent owns the EF Core configuration and migration implications. The verification roles know what evidence they have to demand before a slice can be closed.
...
That is why the handoff schema was upgraded from simple lists to structured objects. Instead of saying only that a prompt may hand off to [`backend-domain`](https://github.com/j0hnnymiller/zeus.academia.3b/blob/main/.github/agents/backend-domain.agent.md) or [`slice-coordinator`](https://github.com/j0hnnymiller/zeus.academia.3b/blob/main/.github/agents/slice-coordinator.agent.md), the handoff now carries a label, a specific agent name, and a short prompt explaining the purpose of the escalation.
...
2. The [`backend-domain`](https://github.com/j0hnnymiller/zeus.academia.3b/blob/main/.github/agents/backend-domain.agent.md) agent implements the aggregate guards, validators, contracts, and bounded behavior.
3. The [`data-persistence`](https://github.com/j0hnnymiller/zeus.academia.3b/blob/main/.github/agents/data-persistence.agent.md) agent decides whether the rule requires database backing and, if it does, adds the EF Core configuration and migration artifact.
4. The [`testing-verification`](https://github.com/j0hnnymiller/zeus.academia.3b/blob/main/.github/agents/testing-verification.agent.md) role runs narrow behavioral checks and also looks for durable evidence in the diff, especially the migration artifact.
5. The [`slice-verifier`](https://github.com/j0hnnymiller/zeus.academia.3b/blob/main/.github/agents/slice-verifier.agent.md) blocks completion if the slice claims schema enforcement but only proves in-memory behavior.
...
## Reviewing the Slice Implementation

On the completion of the first slice, I ran a review against the implementation checklist created in Part 7 by running the prompt: verify the implementation of slice #file:ep-0-1-shared-kernel-implementation.prompt.md. The checklist is designed to capture the key expectations for the foundational slice, especially around durable invariants and migration evidence. Here are the results:

Completion Checklist Gaps
❌ Missing migration artifact — src/backend/Persistence/Migrations/ does not exist. The prompt lists this as a required deliverable.

❌ Shared constants not referenced in EF Core configurations — EmpNr.RequiredLength, Degree.MaxCodeLength, and University.MaxCodeLength exist as constants but are not referenced in AcademicConfiguration.cs or AcademicQualificationConfiguration.cs. Both files use raw literals (6, 10, 15), which can drift from the domain constants.

❌ AcademicQualification.Create does not validate max lengths — Only null/empty is checked. If DegreeCode.Length > 10 or UniversityCode.Length > 10, the domain factory accepts it and the database constraint does the rejection. The prompt requires the factory to reject before hitting the database.

⚠️ No test for DB-level XOR CHECK constraint — SQLite (used in persistence tests) does not enforce CHECK constraints via EF Core. There is no SQL Server migration test to prove the constraint will work against the target provider.

⚠️ Academic.Create uses raw literal 6 and 15 — EmpNr.RequiredLength exists but is not referenced. There is no EmpName.MaxLength constant, so Academic.Create and AcademicConfiguration both embed 15 independently.

Copilot wanted to patch the implementation to fix the issues, but I stopped it and instead chose to run the prompt: suggest changes to the .github guardrails to prevent these gaps from occurring when I rerun the prompt. This produced the following changes to the instruction files, which should help prevent these issues from recurring in future slices and also guide the AI toward better implementation practices around durable invariants and migration evidence.

Three instruction files updated:

**cqrs-mediatr-efcore.instructions.md** — Added two new subsections after `### Change Tracking`:

- **Migrations**: Requires migration artifact in every schema-changing slice diff; mandates `Database.Migrate()` in tests
- **Length and Precision Constants**: Prohibits raw literals in `HasMaxLength`/`HasPrecision`; requires referencing value object constants; notes explicit annotation requirement for provider-sensitive shadow FK types

**csharp-implementation.instructions.md** — Added two checklist items to the Validation Checklist:

- Factory methods validate all persisted constraints against value object constants
- Value object constants are the single source of truth — used in both factory validation and EF Core config

**xunit-implementation.instructions.md** — Replaced the `UseInMemoryDatabase` fixture example with:

- An explicit prohibition on `UseInMemoryDatabase` and `UseSqlite` in-memory for persistence tests
- SQL Server LocalDB fixture using `Database.Migrate()` instead of `EnsureCreated()`

When I reran the implementation prompt, the AI produced code that includes a migration artifact, references value object constants in both domain validation and EF Core configuration, and used a SQL Server LocalDB fixture for persistence tests. The verification agents should also be able to check for these requirements in the diff and block completion if they are not met.

This technique of using the implementation review to tighten the guardrails is a powerful way to turn specific feedback into systemic improvement. Instead of just fixing the code, it updates the instructions that guide future implementations, which should help prevent similar issues from recurring and improve the overall quality of the delivery system.

You can see the updated instruction files in the repository, and the implementation of the Shared Kernel slice now includes durable database constraints, a migration artifact, and sharper agent handoffs as required by the new guidance on the Part-Eight branch of the [Zeus.Academia](https://github.com/j0hnnymiller/zeus.academia.3b) repository.
...
Feedback is always welcome. Send your thoughts to <a href="mailto:AIPractitioner@pdata.com">AIPractitioner@pdata.com</a>.
