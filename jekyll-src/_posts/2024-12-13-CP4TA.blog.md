...
image: /assets/images/2024-12-13/header-CP4TA.jpg
---
...
![Header Image](/assets/images/2024-12-13/header-CP4TA.jpg) If the codebases that you work in are anything like the codebases I work in, test coverage is always lacking in one way or another, if it exists at all. Leveraging AI in test automation is a good way to add value without incurring significant risk.
...
An advantage test code has over application code is that in general, test code is simpler than application code. Typically unit test code exercises the interfaces of the code under test and examines the values returned by the code under test. Effective use of stubs and mocks excludes dependent code further simplifying the test. This makes LLM-generated test automation easier to review for completeness and correctness.

## Test Automation Can Expose Design and Architecture Issues

Asking an LLM to generate test automation for your code has the added advantage of providing an extra "pair of eyes" on your code. To an extent, the quality of the generated test code is correlated to the quality of the code being tested. Tests that are simple and straight forward are an indication that the code is testable. If the LLM has trouble generating tests or produces test that are complex or incomplete, it's a sign that the code is not as testable as it could be and that some refactoring may be in order.

## Enables Refactoring

Of course the major benefit from generating tests with an LLM is that, in the end, you have tests! And having tests is an enabler for refactoring and making improvements to the codebase.
...
There are many ways to generate tests. In this post I'll focus on using GitHub Copilot to demonstrate the ins and outs of generating test automation and I'll discuss Copilot's strengths and weaknesses for this. The concepts are applicable to other LLMs but we won't be comparing them here.
...
To demonstrate generating automated tests we need code to test. More specifically, we need code that is not trivial and that has some degree of complexity to challenge the LLM's test generation capabilities. Rather than trying to extract a snippet from an existing codebase or creating something from scratch, I handed the task off to Copilot.
...
<small><small><small>Original [post](https://blog.pdata.com/CP4TA)</small></small></small>
